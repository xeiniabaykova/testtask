...
TEST(GeomPolyline, Intersect1Point)
{
 ...
 // std::vector<Math::Point> points3 = Math::IntersectPolylinePolyline(polyline3, polyline4);
 // std::vector<Point> resultPoints;
 // for (int i = 0; i < points3.size(); i++)
 // {
 //   resultPoints.push_back(polyline3.GetPoint(points3[i].GetX()));
 // //  EXPECT_TRUE(Math::IsEqual(polyline1.GetPoint(points[i].GetX()), polyline2.GetPoint(points[i].GetY())));
 // }

 // std::vector<Math::Point> points4 = IntersectPolylinePolylineBentleyOttmannAlgorithm(polyline3, polyline4);
 ...
 // points = IntersectPolylinePolylineBentleyOttmannAlgorithm(polyline1, polyline2);
	//EXPECT_FALSE( points.empty() );
 // EXPECT_TRUE(Math::IsEqual(polyline1.GetPoint(points[0].GetX()), polyline2.GetPoint(points[0].GetY())));
