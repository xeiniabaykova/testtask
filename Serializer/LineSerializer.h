...
public:
  /**  \brief Прочитать отрезок из потока.
      Данные читаются в следующем порядке: начало отрезка (Point), конец отрезка (Point).
      В случае невалидных данных возвращается пустой примитив.
  */
  //---
  virtual std::unique_ptr<Math::Curve> Read          ( std::istream& theInput );
  /**  \brief Записать отрезок в поток.
        Данные записываются в следующем порядке: начало отрезка (Point), конец отрезка (Point).
